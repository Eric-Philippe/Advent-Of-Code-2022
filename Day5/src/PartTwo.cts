...
export class PartTwo {
  ...
  constructor() {
    this.table = PartTwo.loadTable();
    this.instructions = PartTwo.loadInstructions();
  }
  ...
  public moveCrate(): Array<Array<string>> {
    for (let i = 0; i < this.instructions.length; i++) {
      ...
      let cratesToMove = this.table[from].splice(
        this.table[from].length - instruction.amountToMove,
        instruction.amountToMove
      );

      // Add the crates to the to column
      this.table[to] = this.table[to].concat(cratesToMove);
    }
