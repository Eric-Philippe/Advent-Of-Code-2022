...
public class PartOne
{
    ...
    public static bool isFullyContained(string str1, string str2) {
        ...
        // Check if the two str are fully contained
        if (str1Min <= str2Min && str1Max >= str2Max) {
            return true;
        }
        return false;
    }
    ...
    */
    public static int processSameRange(string[] input) {
        ...
        for (int i = 0; i < input.Length; i++) {
            ...
            if (isFullyContained(ranges[0], ranges[1])) {
                // If it is, increment the total
                total++;
            } else if (isFullyContained(ranges[1], ranges[0])) {
                // If the second range is fully contained in the first range, increment the total
                total++;
            }
        }
